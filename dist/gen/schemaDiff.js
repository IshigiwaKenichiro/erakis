 ...
 * from → to に変換する操作の分類。
 */
export function diffSchemas(fromFields, toFields) {
    ...
    for (const code of allCodes) {
        ...
        if ((from && SYSTEM_TYPES.has(from.type)) || (to && SYSTEM_TYPES.has(to.type)))
            continue;
        if (!from && to) {
...
function findChangedProps(from, to) {
    const allProps = new Set([...Object.keys(from), ...Object.keys(to)]);
    const changed = [];
    for (const prop of allProps) {
        if (SKIP_DIFF_PROPS.has(prop))
            continue;
 ...
 */
export function formatDiffSummary(result, fromLabel, toLabel, appName = '', fromFields, toFields) {
    const lines = [`差分: ${fromLabel} → ${toLabel}`, ''];
        ...
        for (const d of result.update) {
            ...
            if (fromFields && toFields && d.changedProps && d.changedProps.length > 0) {
                ...
                for (const prop of d.changedProps) {
                    const devVal = fromField[prop];
                    const prodVal = toField[prop];
                    lines.push(`    ${prop}:`);
 ...
 */
export function formatFullDiff(fromFields, toFields, result) {
    const sections = [];
    const allChanged = [
        ...result.update,
        ...result.typeChange,
    ...
    for (const d of allChanged) {
        ...
        const patch = createPatch(d.code, fromJson, toJson, 'dev', 'prod');
...
function normalizeFieldJson(field) {
