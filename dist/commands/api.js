...
export function apiCommand() {
    ...
    sub.command('gen')
        ...
        .option('--app <appId:alias>', 'target app(s) in appId:alias format (repeatable)', (val, acc) => { acc.push(val); return acc; }, [])
 ...
 */
function validateProfiles(resolved) {
...
async function gen(options) {
    ...
    if (options.app && options.app.length > 0) {
        ...
        const resolved = apiStorage.getResolvedTargets({ profile: options.profile, out: options.out }, filterAliases);
        // ターゲット一覧表示（全ターゲット）
    ...
    const resolved = apiStorage.getResolvedTargets({ profile: options.profile, out: options.out });
