...
import { schemaToDigestRows, formatDigestMarkdown, formatDigestTsv, } from '../../gen/digestGenerator.js';
/**
 * 未登録 appId の ID ベースのファイル stem を返す純粋関数。
 * ファイル名に使うため、kintone アプリ名ではなく ID で固定する。
 */
export function rawFileStem(appId, guestSpaceId) {
 ...
 */
export function buildHelpHint(appRef) {
    return [
 ...
 */
async function digestRawApp(appId, guestSpaceId, options) {
    const { profiles } = profileStorage.getData();
    const { customizations } = appStorage.getData();
    const profileResult = resolveProfileNameForAppId(appId, customizations, Object.keys(profiles), options.profile);
    ...
    try {
        ...
        const revision = String(fieldsResp.revision ?? '');
        const meta = {
            appName: displayName,
            environment: 'raw',
            appId,
 ...
 */
export function classifyAppOption(app, registeredNames) {
    if (!app)
        return { type: 'all' };
    // 登録済みアプリ名が最優先
    if (registeredNames.includes(app))
        return { type: 'registered' };
    // 次に数値 appId として解釈を試みる
    const raw = parseRawAppRef(app);
    if (raw)
        return { type: 'raw', appId: raw.appId, guestSpaceId: raw.guestSpaceId };
 ...
 */
export async function schemaDigest(options) {
    const { customizations } = appStorage.getData();
 ...
 */
export async function writeDigest(appName, fields, layout, meta, format = 'md', env = 'dev', appRefForHelp) {
