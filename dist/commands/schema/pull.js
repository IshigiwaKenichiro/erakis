 ...
 */
export async function schemaPull(options) {
    ...
    for (const appName of targetApps) {
        ...
        const manifestEntry = { appName };
        for (const envKey of ['dev', 'prod']) {
            ...
            try {
                ...
                const [fieldsResp, layoutResp, viewsResp] = await Promise.all([
                    ...
                    client.app.getViews({ app: env.appId, preview: true }),
                ]);
                const sortedFields = {};
                ...
                fs.writeJSONSync(path.join(appDir, 'layout.json'), layoutResp.layout, { spaces: '\t' });
                fs.writeJSONSync(path.join(appDir, 'views.json'), viewsResp.views, { spaces: '\t' });
                const meta = {
                    appName,
                    environment: envKey,
                    appId: env.appId,
    ...
    if (options.latest !== false)
        console.log(chalk.gray(`  latest.json updated`));
}
