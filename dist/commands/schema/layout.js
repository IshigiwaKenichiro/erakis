 ...
 */
export function resolveLayoutSyncEnvs(toEnv) {
    if (toEnv === 'dev')
        return { from: 'prod', to: 'dev' };
    if (toEnv === 'prod')
        return { from: 'dev', to: 'prod' };
 ...
 */
export function parseLayoutInput(raw) {
    if (Array.isArray(raw))
        return raw;
    if (raw !== null && typeof raw === 'object' && Array.isArray(raw.layout)) {
        return raw.layout;
    }
 ...
 * LIVE preview 読み取り専用。
 */
export async function schemaGetLayout(appRef, filePath, options) {
    if (!appRef) {
    ...
    if (!ref)
        return;
    try {
        const resp = await ref.client.app.getFormLayout({ app: ref.appId, preview: true });
 ...
 */
export async function schemaUpdateLayout(appRef, filePath, options) {
    if (!appRef) {
        console.error(chalk.red('app ref is required.'));
 ...
 */
export async function schemaSyncLayout(appName, options) {
    if (!appName) {
    ...
    let layout;
    try {
        const resp = await srcRef.client.app.getFormLayout({ app: srcRef.appId, preview: true });
        layout = resp.layout;
    }
    catch (e) {
        const msg = e instanceof Error ? e.message : String(e);
