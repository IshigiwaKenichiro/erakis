 ...
 */
export function resolveViewsSyncEnvs(toEnv) {
    if (toEnv === 'dev')
 ...
 */
export function parseViewsInput(raw) {
 ...
 * LIVE preview 読み取り専用。
 */
export async function schemaGetViews(appRef, filePath, options) {
    if (!appRef) {
    ...
    if (!ref)
        return;
    try {
        const resp = await ref.client.app.getViews({ app: ref.appId, preview: true });
 ...
 */
export async function schemaUpdateViews(appRef, filePath, options) {
    if (!appRef) {
        console.error(chalk.red('app ref is required.'));
 ...
 */
export async function schemaSyncViews(appName, options) {
    if (!appName) {
    ...
    let views;
    try {
        const resp = await srcRef.client.app.getViews({ app: srcRef.appId, preview: true });
        views = sanitizeViewsForUpdate(resp.views ?? {});
    }
    catch (e) {
        const msg = e instanceof Error ? e.message : String(e);
