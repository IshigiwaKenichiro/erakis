 ...
 */
function assertLaunchFilesReady(appName, status) {
    if (status === 'local') {
        ...
        if (missing.length > 0) {
            throw new Error(`"${appName}" is missing local customization files needed for localhost URLs.\n` +
 ...
 */
export async function mergeCustomize(appName, app) {
    ...
    const customize = {
        scope: 'ALL',
        desktop: {
            js: (cdnJson.desktop?.js ?? []).map((url) => ({ type: 'URL', url })),
            css: (cdnJson.desktop?.css ?? []).map((url) => ({ type: 'URL', url }))
        },
        mobile: {
            js: (cdnJson.mobile?.js ?? []).map((url) => ({ type: 'URL', url })),
            css: (cdnJson.mobile?.css ?? []).map((url) => ({ type: 'URL', url }))
        }
    };
    if ("local" == app.status) {
...
function fileCustom(target, fileName, contentType, fileKey, size) {
...
function addCustom(target, fileName, url) {
    const idx = target.findIndex(js => {
        switch (js.type) {
            case 'URL': return js.url.includes(fileName);
            case 'FILE': return js.file.name.includes(fileName);
