 ...
 */
export function parseSpaceRef(arg: string | undefined): SpaceRef | null {
 ...
 */
export function resolveProfileForCreate(
    ...
    if (allProfileNames.length === 0) return { error: 'no profiles found. run `erakis profile add` first.' };
 ...
 */
export async function schemaCreate(spaceIdOrGuestSpaceId: string | undefined, options: CreateOptions) {
    ...
    const params: Record<string, unknown> = { name: 'erakis app' };
    if (parsed.id) params.space = Number(parsed.id);

    const result = await (client.app as any).addApp(params);
