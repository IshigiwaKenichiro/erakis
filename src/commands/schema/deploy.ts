...
import { deployAndWait, formatDeployStatus, DivergenceState, DeployStatusEntry, checkSchemaDivergence } from '../../utils/schemaWriter.js';
 ...
 */
export async function schemaDeploy(
    appRef: string | undefined,
    options: { profile?: string; env?: string },
): Promise<void> {
    if (!appRef) {
 ...
 */
export async function schemaUndeploy(
    appRef: string | undefined,
    options: { profile?: string; env?: string; yes?: boolean },
): Promise<void> {
    if (!appRef) {
        console.error(chalk.red('app ref is required.'));
 ...
 */
export async function schemaGetDeployStatus(
    appRef: string | undefined,
    options: { profile?: string; env?: string },
): Promise<void> {
    if (!appRef) {
    ...
    try {
        ...
        const rawEntries: { app: string; status: string }[] = (resp.apps ?? []).map((a: any) => ({
            app: String(a.app),
            status: String(a.status),
        }));

        // PROCESSING 以外は live/preview を実比較して divergence を付与する
        const entries: DeployStatusEntry[] = await Promise.all(rawEntries.map(async (e): Promise<DeployStatusEntry> => {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            const line = lines[i];
            if (!entry) { console.log(line); continue; }
