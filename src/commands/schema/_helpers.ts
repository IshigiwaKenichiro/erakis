 ...
 */
export async function fetchLiveAppSchema(
    appName: string,
    envKey: 'dev' | 'prod',
): Promise<{ fields: Record<string, any>; layout: any[]; revision: string } | null> {
    ...
    try {
        ...
        return {
            ...
            revision: String((fieldsResp as any).revision ?? ''),
        };
    } catch (error) {
 ...
 */
export async function fetchLiveRevision(client: any, appId: string): Promise<string | null> {
    try {
        const resp = await client.app.getFormFields({ app: appId, preview: true });
        const revision = String((resp as any).revision ?? '');
 ...
 * @param envKey  - 'dev' または 'prod' 環境
 */
export async function fetchLiveViews(appName: string, envKey: 'dev' | 'prod'): Promise<Record<string, any> | null> {
    const { customizations } = appStorage.getData();
    const { profiles } = profileStorage.getData();
    const custom = customizations[appName];
    if (!custom) {
    ...
    try {
        const client = createKintoneClient(profile, { guestSpaceId: env.guestSpaceId || undefined });
        const resp = await client.app.getViews({ app: env.appId, preview: true } as any);
        return (resp as any).views;
    } catch (error) {
        const msg = error instanceof Error ? error.message : String(error);
 ...
 */
export function checkRevisionResult(
 ...
 */
export function parseRawAppRef(arg: string): { appId: string; guestSpaceId: string | undefined } | null {
 ...
 */
export function resolveProfileNameForAppId(
    appId: string,
    ...
    for (const custom of Object.values(customizations)) {
        for (const env of [custom.development, custom.production]) {
            if (env.appId === appId) {
                return { profileName: env.profileName as string };
            }
        }
    }
    // 2. 明示的なヒント
    if (profileHint) return { profileName: profileHint };
    // 3. プロファイルが1件の場合のフォールバック
    if (allProfileNames.length === 1) return { profileName: allProfileNames[0] };
