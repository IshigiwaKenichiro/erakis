...
import type { Customization, Profile } from '../models.js';
...
async function createGinueRc(): Promise<void> {
...
function generateGinueConfig(customizations: { [appName: string]: Customization }, profiles: { [name: string]: Profile }): string {
    ...
    // 環境別設定を生成
    const envConfigs: { [envName: string]: any } = {};
...
function createEnvConfig(profile: Profile, app: any) {
    return {
