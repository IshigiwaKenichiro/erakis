 ...
 */
export function url2ids(urlstr: string, profile: Profile) {
    const ret = {
...
export async function promptEnv() {
    ...
    const { appUrl } = await inq.prompt([
        {
            ...
            validate: (input) => {
                ...
                if (0 < errors.length) return errors.join(',');
                return true;
            }
        }
    ]);
    ...
    return { profileName: profile.name, appId, guestSpaceId, status: 'local', baseUrl: profile.baseUrl } as App;
}
 ...
 */
export function application2url(application: App) {
    const { profiles } = profileStorage.getData();
 ...
 */
export function getChalk(app: App) {
    switch (app.status) {
 ...
 * kintone API 呼び出しなし。
 */
export function resolveEnvFromOptions(profileName: string, appUrl: string): App | null {
    const { profiles } = profileStorage.getData();
    const profile = profiles[profileName];
