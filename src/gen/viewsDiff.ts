...
export type ViewsDiffResult = {
    add: string[];
    delete: string[];
    change: { name: string; changedProps: string[] }[];
};

function normalizeView(view: Record<string, any>): Record<string, any> {
    const { id: _id, ...rest } = view;
    return rest;
}

function findChangedProps(dev: Record<string, any>, prod: Record<string, any>): string[] {
...
export function diffViews(
    devViews: Record<string, any>,
    prodViews: Record<string, any>,
): ViewsDiffResult {
    ...
    const change: { name: string; changedProps: string[] }[] = [];
    for (const name of [...devNames].filter(n => prodNames.has(n)).sort()) {
        ...
        if (changedProps.length > 0) change.push({ name, changedProps });
    }
 ...
 */
export function formatViewsDiffReport(result: ViewsDiffResult): string {
    const total = result.add.length + result.delete.length + result.change.length;
    if (total === 0) return 'Views 差分なし';

    const lines: string[] = [
        `## Views diff: 追加 ${result.add.length}件 / 削除 ${result.delete.length}件 / 変更 ${result.change.length}件`,
    ];
    for (const name of result.add) lines.push(`  + ${name}`);
    for (const name of result.delete) lines.push(`  - ${name}`);
    for (const { name, changedProps } of result.change) lines.push(`  ~ ${name} (${changedProps.join(', ')})`);
 ...
 */
export function formatFullViewsDiff(
    devViews: Record<string, any>,
    prodViews: Record<string, any>,
): string {
    const normalize = (views: Record<string, any>) =>
        JSON.stringify(
            Object.fromEntries(Object.keys(views).sort().map(k => [k, normalizeView(views[k])])),
            null,
            2,
        );
