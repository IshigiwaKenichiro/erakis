 ...
 */
async function fetchSchema(client: KintoneRestAPIClient, appId: string): Promise<FieldInfo[]> {
...
function genSharedResult(): string {
...
function genSharedClient(): string {
    return `// @generated - このファイルは自動生成されます。手動で編集しないでください。
import { KintoneRestAPIClient } from '@kintone/rest-api-client';
...
function genTypes(alias: string, fields: FieldInfo[]): string {
    ...
    for (const f of fields) {
        if (READ_ONLY_TYPES.has(f.type)) continue;
        const valueType = FIELD_VALUE_TYPE[f.type] ?? 'string';
...
function genOperations(alias: string): string {
    const typeName = toPascalCase(alias);
...
function genIndex(alias: string): string {
    const typeName = toPascalCase(alias);
 ...
 */
function quoteFieldName(code: string): string {
 ...
 * 空ディレクトリも再帰的に削除
 */
function cleanStaleFiles(outDir: string, generatedPaths: Set<string>, dryRun: boolean, verbose: boolean): number {
 ...
 */
async function loadPrettier(): Promise<{ format: (source: string, options: Record<string, unknown>) => Promise<string> } | null> {
    try {
...
export async function generateApi(ctx: GenerateContext): Promise<{ success: boolean; generated: number; errors: string[] }> {
    const errors: string[] = [];
    let generated = 0;
