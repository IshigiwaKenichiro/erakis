...
export type LayoutElementKind = 'field' | 'spacer' | 'label' | 'hr' | 'group' | 'subtable';
...
export type LayoutItemDiff = {
    category: 'add' | 'delete' | 'move' | 'change';
    key: string;
    kind: LayoutElementKind;
    devElem?: LayoutElement;
    prodElem?: LayoutElement;
};

export type LayoutDiffResult = {
    add: LayoutItemDiff[];
    delete: LayoutItemDiff[];
    move: LayoutItemDiff[];
    change: LayoutItemDiff[];
};

// ---- 要素抽出 ----

function kindOf(item: any): LayoutElementKind {
    switch (item.type) {
 ...
 */
export function flattenLayout(layout: any[]): LayoutElement[] {
    ...
    function addElem(
        ...
        row: number,
        col: number,
        opts?: { innerRow?: number; innerCol?: number; group?: string; parentTable?: string }
    ): void {
    ...
    for (let rowIdx = 0; rowIdx < layout.length; rowIdx++) {
        ...
        if (item.type === 'ROW') {
            ...
            const childFields: any[] = Array.isArray(rawFields)
                ? rawFields
                : Object.entries(rawFields).map(([code, f]) => ({ code, ...(f as any) }));
...
function samePosition(dev: LayoutElement, prod: LayoutElement): boolean {
...
function comparableContent(elem: LayoutElement): any {
    const raw = elem.raw;
    if (elem.kind === 'group') {
...
function sortObjKeys(obj: any): any {
    if (Array.isArray(obj)) return (obj as any[]).map(sortObjKeys);
 ...
 */
export function diffLayouts(devLayout: any[], prodLayout: any[]): LayoutDiffResult {
    const result: LayoutDiffResult = { add: [], delete: [], move: [], change: [] };
...
function positionStr(elem: LayoutElement): string {
    if (elem.group) {
...
function extraDesc(elem: LayoutElement): string {
    const r = elem.raw;
    if (elem.kind === 'label' && r.label) return `, text: "${r.label}"`;
    if (elem.kind === 'spacer' && r.elementId) return `, elementId: "${r.elementId}"`;
    return '';
}
 ...
 */
export function formatLayoutDiffReport(result: LayoutDiffResult): string {
    const lines: string[] = [];
    ...
    if (result.add.length > 0) {
        ...
        for (const d of result.add) {
            ...
            if (d.prodElem) lines.push(`    prod: ${positionStr(d.prodElem)}${extraDesc(d.prodElem)}`);
        }
        lines.push('');
    }
    ...
    if (result.delete.length > 0) {
        ...
        for (const d of result.delete) {
            lines.push(`  - ${d.key} [${d.kind}]`);
            if (d.devElem) lines.push(`    dev: ${positionStr(d.devElem)}${extraDesc(d.devElem)}`);
        }
        lines.push('');
    }
    ...
    if (result.move.length > 0) {
        ...
        for (const d of result.move) {
            ...
            if (d.devElem) lines.push(`    dev: ${positionStr(d.devElem)}`);
            if (d.prodElem) lines.push(`    prod: ${positionStr(d.prodElem)}`);
        }
        lines.push('');
    }
