 ...
 */
export function buildDevToProdMap(
 ...
 */
export function remapLookupAppIds(
    fields: Record<string, any>,
    devToProdMap: Map<string, string>,
): RemapResult {
    const remapped: Record<string, any> = JSON.parse(JSON.stringify(fields));
    ...
    for (const [code, field] of Object.entries(remapped)) {
        ...
        if (f.type === 'REFERENCE_TABLE' && f.referenceTable?.relatedApp?.app) {
            ...
            if (prodId) {
                ...
                if (!mappedCodes.includes(code)) mappedCodes.push(code);
            } else {
                if (!unmappedCodes.includes(code)) unmappedCodes.push(code);
            }
        }

        // SUBTABLE 子フィールドの lookup / REFERENCE_TABLE も再帰的にリマップ
        if (f.type === 'SUBTABLE' && f.fields) {
            for (const [childCode, childField] of Object.entries(f.fields as Record<string, any>)) {
                const cf = childField as any;
                if (cf.lookup?.relatedApp?.app) {
                    const relId = String(cf.lookup.relatedApp.app);
                    const prodId = devToProdMap.get(relId);
                    if (prodId) {
                        cf.lookup.relatedApp.app = prodId;
                        if (!mappedCodes.includes(childCode)) mappedCodes.push(childCode);
                    } else {
                        if (!unmappedCodes.includes(childCode)) unmappedCodes.push(childCode);
                    }
                }
 ...
 * 不正なコードの配列を返す（空 = OK）。
 */
export function validateLayoutCodes(
    layout: any[],
    fields: Record<string, any>,
): string[] {
    // 全フィールドコードを収集（SUBTABLE 子も含む）
    const allCodes = new Set<string>(Object.keys(fields));
    for (const field of Object.values(fields)) {
        if ((field as any).type === 'SUBTABLE' && (field as any).fields) {
            for (const childCode of Object.keys((field as any).fields)) {
 ...
 */
export function sanitizeViewsForUpdate(views: Record<string, any>): Record<string, any> {
    const result: Record<string, any> = {};
    for (const [name, view] of Object.entries(views)) {
        const { id: _id, ...rest } = view as any;
        result[name] = rest;
    }
    return result;
}
...
export async function deployAndWait(
    ...
    const appParam: any = { app: appId };
    if (revision != null) appParam.revision = String(revision);
    const deployOptions: any = { apps: [appParam] };
    if (options?.revert) deployOptions.revert = true;
    ...
    while (true) {
        ...
        const entry = resp.apps?.find((a: any) => String(a.app) === String(appId));
        if (!entry) throw new Error(`deploy status not found for app ${appId}`);
        if (entry.status === 'SUCCESS') return;
        if (entry.status === 'FAIL') throw new Error(`deploy FAIL for app ${appId}`);
        if (entry.status === 'CANCEL') throw new Error(`deploy CANCEL for app ${appId}`);
        if (Date.now() - start > DEPLOY_TIMEOUT_MS) throw new Error(`deploy timeout for app ${appId}`);
    }
}

/** スキーマ比較結果: 'no-diff'=差分なし / 'has-diff'=差分あり / 'fetch-failed'=比較取得失敗 */
export type DivergenceState = 'no-diff' | 'has-diff' | 'fetch-failed';

export type DeployStatusEntry = {
    app: string;
    status: string;
    /** PROCESSING 以外の場合に設定される。undefined はフォールバック */
 ...
 */
export function formatDeployStatus(entries: DeployStatusEntry[]): string {
 ...
 */
export function normalizeForCompare(obj: unknown): string {
    if (obj === null || typeof obj !== 'object') return JSON.stringify(obj);
    if (Array.isArray(obj)) return '[' + (obj as unknown[]).map(normalizeForCompare).join(',') + ']';
    const keys = Object.keys(obj as object).sort();
    return '{' + keys.map(k => JSON.stringify(k) + ':' + normalizeForCompare((obj as Record<string, unknown>)[k])).join(',') + '}';
}
 ...
 */
export async function checkSchemaDivergence(client: any, appId: string): Promise<DivergenceState> {
    try {
        ...
        if (hasFieldDiff) return 'has-diff';

        if (normalizeForCompare(liveLayout.layout) !== normalizeForCompare(previewLayout.layout)) return 'has-diff';

        const liveViewsSan = sanitizeViewsForUpdate((liveViews as any).views ?? {});
        const previewViewsSan = sanitizeViewsForUpdate((previewViews as any).views ?? {});
        if (normalizeForCompare(liveViewsSan) !== normalizeForCompare(previewViewsSan)) return 'has-diff';
...
/** launch 時のスキーマ状態警告行 */
export type LaunchWarningLine = { text: string };
 ...
 * 警告不要の場合は null を返す。
 */
export function formatLaunchSchemaWarning(
    appName: string,
    deployStatus: string | null,
    divergence?: DivergenceState,
    env?: string,
): LaunchWarningLine[] | null {
    const lines: LaunchWarningLine[] = [];
 ...
 */
export async function checkRemoteRevision(
    client: any,
    appId: string,
    localRevision: string,
): Promise<{ remoteRevision: string; stale: boolean }> {
    const resp = await client.app.getFormFields({ app: appId, preview: true });
    const remoteRevision = String((resp as any).revision ?? '');
